...
*       ----------------------------------------------------------------      */
        ...
        )
    {
        ...
        if (init == 'n')
        {
            ...
            else
            {
                ...
                alfdp = alfdp + dalf;
                betdp = betdp + dbet;
                rec->nodep = sgp4_mod(rec->nodep, twopi);
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        xnodce = sgp4_mod(4.5236020 - 9.2422029e-4 * rec->day, twopi);
        stem = sin(xnodce);
        ctem = cos(xnodce);
        ...
        rec->zmol = sgp4_mod(4.7199672 + 0.22997150  * rec->day - rec->gam, twopi);
        rec->zmos = sgp4_mod(6.2565837 + 0.017201977 * rec->day, twopi);
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        rec->dndt = 0.0;
        theta = sgp4_mod(rec->gsto + tc * rptim, twopi);
        ...
        if (rec->irez != 0)
        {
            ...
            if (rec->irez == 2)
            {
                ...
                rec->xlamo = sgp4_mod(rec->mo + rec->nodeo + rec->nodeo - theta - theta, twopi);
            ...
            if (rec->irez == 1)
            {
                ...
                rec->xlamo = sgp4_mod(rec->mo + rec->nodeo + rec->argpo - theta, twopi);
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    void initl(double epoch, ElsetRec *rec)
    {
        ...
        c1p2p = c1 + twopi;
        double gsto1 = sgp4_mod(thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, twopi);
        if (gsto1 < 0.0)
            gsto1 = gsto1 + twopi;
        //    }
        //    else
    ...
    *    vallado, crawford, hujsak, kelso  2006
        ...
        )
    {
        ...
        satrec->nodem = sgp4_mod(satrec->nodem, twopi);
        satrec->argpm = sgp4_mod(satrec->argpm, twopi);
        xlm = sgp4_mod(xlm, twopi);
        satrec->mm = sgp4_mod(xlm - satrec->argpm - satrec->nodem, twopi);
        ...
        u = sgp4_mod(xl - satrec->nodep, twopi);
        eo1 = u;
        tem5 = 9999.9;
        ktr = 1;
        sineo1 = 0;
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    double sgp4_mod(double numer, double denom)
    {
    ...
    * --------------------------------------------------------------------------- */
    ...
    double gstime(double jdut1)
    {
        ...
        temp = sgp4_mod(temp * deg2rad / 240.0, twopi); //360/86400 = 1/240, to deg, to rad
