...
{
    downlink=false;
    // SDR Town owns aircraft metadata; decoding never waits for a database thread.
...
AeroL::AeroL(QObject *parent) : QIODevice(parent)
{
    ...
    // SDR Town calls advanceSecond on the sample clock (fast replay == live).
        ...
        if(cntr>=16)
        {
            ...
            if(burstmode)
            {
                ...
                case RTChannelDeleaveFECScram::OK_T_Packet:
                {
                    ...
                    decodedbytes+=decline.toUtf8();decline.clear();
                   ...
                   //delay line for frame alignment for non burst modes. This is needed for the scrambler
                    ...
                    if((cntr-AERO_SPEC_BitsInHeader)==(AERO_SPEC_NumberOfBits-1))//frame is done when this is true
                    {
                        ...
                        for(int k=0;k<infofield.size()/12;k++)
                        {
                            ...
                            // Zero padding is not a CRC-valid signal unit and cannot establish lock.
                            ...
                            emit CrcResult(crc_calc==crc_rec);
                            if(crc_calc==crc_rec)
                            {
                                emit ValidatedSignalUnit(infofield.mid(k*12,12));
            ...
            if(cntr==AERO_SPEC_NumberOfBits-1)
            {
                ...
                for(int y=0; y<24;y++)
                {
                    ...
                    if(infofield.size()==12)
                    {
                        ...
                        {
                            ++validUnits;
                            emit ValidatedSignalUnit(infofield);
                            crcok = true;
