...
QVector<int> &JConvolutionalCodec::Decode_hard(const QByteArray& soft_bits_in, int size)//0-->-1 128-->0 255-->1
{
    ...
    //decoce
    decoded.fill(0,(hard_bytes_in.size()+nparitybits-1)/nparitybits);
    ...
    {
        uchar uch=decoded[i];
        for(int k=0;k<8 && bit_ptr<static_cast<int>(dbits);k++)
        {
...
{
    if(size<0 || size>soft_bits_in.size()) throw std::invalid_argument("Aero convolutional input length");

    soft_bits_overlap_buffer_uchar.clear();
    soft_bits_overlap_buffer_uchar.append(soft_bits_in);

    //decode
    decoded.fill(0,size/nparitybits+1);
...
QVector<int> &JConvolutionalCodec::Decode_Continuous(QByteArray& soft_bits_in)//0-->-1 128-->0 255-->1
{
    ...
    decoded.fill(0,(soft_bits_overlap_buffer_uchar.size()/nparitybits)+1);
