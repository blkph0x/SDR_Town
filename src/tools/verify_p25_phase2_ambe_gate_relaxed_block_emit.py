...
text = main
fn = text[text.index('bool p25AmbeDecodeFrameLooksUsable'):text.index('QString p25Phase2ValidationPath')]
...
assert 'bool p25DecodedAmbePcmLooksSafeForSpeaker' in fn, 'speaker gate must be separate from strict fresh-speech proof'
...
decode = text[text.index('bool p25DecodePhase2AmbeFrameToAudio'):text.index('bool p25ProbePhase2AmbeFrameForDiagnostics')]
...
may = text[text.index('bool p25VoiceBlockMayEmitAudio'):text.index('P25VoiceAudioBlock applyP25Phase2SecurityAudioGate')]
