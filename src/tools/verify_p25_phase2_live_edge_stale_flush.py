...
assert 'phase2IqByRx.erase(&rx);' in s, 'GUI/CLI must clear rolling IQ buffer on stale no-sync'
assert 'lastPhase2DecodeByRx.erase(&rx);' in s, 'GUI/CLI must reset decode cadence after stale no-sync'
assert 'pendingAudioByRx[&rx].clear();' in s, 'GUI/CLI must clear pending PCM on stale no-sync'
