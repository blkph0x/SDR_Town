...
assert (
    'rolling.commitDecodeAbsolute(iqDecodeEndAbsolute)' in main
    or 'phase2IqByRx[p25ReceiverSessionKey(rx)].commitDecodeAbsolute(iqDecodeEndAbsolute)' in main
), 'inline replay path must commit returned chunk end'
assert (
    'rollingIt->second.commitDecodeAbsolute(result.iqDecodeEndAbsolute)' in main
), 'worker result must commit returned chunk end'
