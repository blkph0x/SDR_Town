...
device_manager = (root / 'src' / 'DeviceManager.cpp').read_text(errors='ignore')
...
checks = {
    'rolling IQ tiny-fragment guard': 'freshSamples < effectiveMinFresh' in main and 'p25Phase2EffectiveMinFreshSamples' in main,
    'minimum fresh IQ passed at GUI decode': '&phase2FreshIqSamples, &phase2ContextIqSamples, minDecodeFresh' in main,
    'final stale traffic generation drop': 'dropStaleTrafficAudio' in main and 'rx.p25TrafficGeneration != result.trafficGeneration' in main and 'liveGen != result.trafficGeneration' in main,
    ...
    'waitgrant grant priority prevents lower-confidence overwrite': 'selectedGrantScore' in main and 'candidateScore <= selectedGrantScore' in main and 'probingUnknownPhase2EncryptedHistory' in main,
    'CLI follow snapshot carries speaker/session evidence': 'followSnapshot.recentSpeakerOutputMs = lastSpeakerOutputMs;' in main and 'cliCurrentCallSessionId = rx.p25CurrentCallSessionId;' in main,
    'Phase 2 continuation cannot re-enter fresh acquire': '!phase2RecentContinuation &&' in fsm and 'const bool phase2RecentContinuation' in fsm,
    'Soapy timeout logging is rate limited': 'consecutiveReadTimeouts' in device_manager and 'msSinceReadErrorLog >= 5000' in device_manager and 'readStream timeout on device' in device_manager,
}
