...
may = text.split("bool p25Phase2MayAppendPlcBlock", 1)[1].split(
    "bool p25Phase2AudioTailGraceActive", 1)[0]
gate = text.split("std::string p25VoiceBlockSpeakerGateReason", 1)[1].split(
    "bool p25VoiceBlockMayEmitAudio", 1)[0]
opp = text.split("void p25Phase2AppendOppositeSlotSustainPlc", 1)[1].split(
    "bool p25Phase2AmbeProbeScoreLooksFinite", 1)[0]
gap = text.split("void p25Phase2FillFeedGapWithPlc", 1)[1].split(
    "void p25RecordPhase2AmbeFeedCadence", 1)[0]
...
checks = {
    ...
    "sticky mac cannot feed established clear noise": (
        ...
        "p25Phase2CurrentSelectedBurstFeedTrusted(burst)" in text.split(
            "bool p25Phase2EstablishedClearNoiseFeedAllowed", 1
        )[1].split("void p25Phase2UpdateAudioTailTracker", 1)[0]
    ),
    ...
    "no rf-gap plc injection": "p25RecordPhase2AmbeFeedCadence(P25VoiceAudioBlock& out" in text and
                               ...
                               text.split("void p25RecordPhase2AmbeFeedCadence", 1)[1].split("static json p25Phase2EssJson", 1)[0],
