...
checks = {
    ...
    'XOR mask phase must not relabel TDMA traffic slot': 'trafficSuperframeBurstIndex' in p25 and 'maskSuperframeBurstIndex' in p25 and 'burst.grantSlot=phase2TrafficSlotFromSuperframeBurstIndex(trafficSuperframeBurstIndex)' in p25_compact and 'phase2TrafficSlotFromSuperframeBurstIndex(maskSuperframeBurstIndex)' not in p25 and 'window.sessions[trafficSlot & 0x01u]' in p25 and 'slotSessions[trafficSlot & 0x01u]' in p25,
    ...
    'Unknown audio queues by target call key before release': 'if (unknownSecurity)' in main and 'p25QueuePhase2PendingAmbeFrame(rx, audioKey, pending)' in main and 'p25TakePhase2PendingAudio(rx,audioKey,kP25Phase2PendingDrainMaxFramesPerTick)' in main_compact,
    'Explicit clear grant queues until target traffic proves clear': 'cleargranttargetreleaseallowed' not in main_l and 'p25phase2explicitcleargrantvoicereleaseevidence' in main_l and 'releasePendingRawVoiceFromExplicitClearTrafficProof' in main and 'p25Phase2ExplicitClearGrantTargetMacTrafficProof' in main and 'explicit-clear-grant-target-release' not in main and 'explicit-clear-grant-validated-release' not in main and 'explicit-clear-grant-traffic-clear-release' in main and 'validatedExplicitClearGrantSeed' not in main and 'boundedProbeAccepted' not in main and 'const bool grantMayProbeVoice = grantClearTrusted || grantUnknownProbe;' in main and 'establishedClearCall' in main and 'const bool freshTargetTrafficClearEvidence =' in main and 'p25Phase2ExplicitClearGrantTargetMacTrafficProof(rx, out)' in fresh_traffic_expr and 'const bool recentClearContinuationEvidence =' in main and 'const bool targetTrafficClearEvidence =' in main and 'freshTargetTrafficClearEvidence || recentClearContinuationEvidence' in main and 'p25Phase2TargetHardClearEvidence(out)' in main and 'p25Phase2TargetPttSessionClear' in main and 'p25Phase2SdrtrunkLateEntryVoiceReleaseEvidence(rx, out)' in main and 'p25Phase2ExplicitClearGrantTargetMacTrafficProof(rx, out)' in main and 'explicitGrantTargetSlotSelected' in main and 'explicitClearGrantSelectedVoiceRelease' not in main and '(targetTrafficClearEvidence || explicitClearGrantSelectedVoiceRelease)' not in main and '(targetTrafficClearEvidence || explicitGrantTargetSlotSelected)' not in main and 'freshTargetTrafficClearEvidence &&' in main.split('const bool explicitClearGrantHardVoiceRelease', 1)[1].split(';', 1)[0],
