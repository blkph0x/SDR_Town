...
assert 'kP25Phase2VoiceDecodeFirstColdEyeSeconds = 0.360' in text, 'First traffic eye should cover a full Phase 2 superframe'
assert 'kP25Phase2VoiceDecodeAcquireChunkSeconds = 0.050' in text, 'Post-eye acquire should use short bounded fresh chunks'
assert 'kP25Phase2VoiceDecodeAcquireOverlapSeconds = 0.050' in text, 'Acquire window should use bounded pre-roll while cold locking'
...
assert 'kP25Phase2VoiceDecodeSustainOverlapSeconds = 0.600' in text, 'Sustain must preserve multi-superframe pre-roll for continuous AMBE'
assert 'kP25Phase2VoiceDecodeSpeakerSustainOverlapSeconds = 0.640' in text, 'Speaker sustain must keep large pre-roll after audio opens'
