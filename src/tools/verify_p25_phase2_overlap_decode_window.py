...
assert 'firstColdEyeChunk || unacquiredAcquireWindow' in text, 'Unacquired acquisition windows must be treated as fresh evidence, not stale context'
...
assert 'kP25Phase2VoiceDecodeSpeakerSustainChunkSeconds = 0.720' in text, 'Speaker sustain must keep cold-eye sized fresh IQ (not 40 ms crumbs)'
assert 'kP25Phase2VoiceDecodeSpeakerSustainOverlapSeconds = 0.060' in text, 'Speaker sustain uses short lattice pre-roll with full-eye fresh chunks'
assert 'kP25Phase2VoiceDecodeSpeakerSustainMinFreshSeconds = 0.360' in text, 'Speaker sustain min fresh must cover multi-burst PCM'
