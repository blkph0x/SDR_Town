...
checks = {
    ...
    'direct CRC returns parseable MAC PDU': 'directCrcOk && phase2DirectCrcMacPduLooksSdrtrunkParseable(directPdu, macStructureMaxBits)' in text and 'return directPdu;' in text,
    'RS failure returns diagnostic candidate': 'if (directDiagnosticPdu) return directDiagnosticPdu;' in text,
    'decoded PDU carries SDRTrunk parse limit': 'pdu.macStructureMaxBits = macStructureMaxBits;' in text,
