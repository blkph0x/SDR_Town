...
- **P25 CC**: the P25 control-channel section lists likely 12.5 kHz-ish candidates from the live FFT; double-click a candidate to tune it. Monitoring a CC now mutes raw discriminator audio, while the **Auto Follow Grants** checkbox watches the monitored control channel, follows clear talkgroup voice grants, and returns to the muted control channel when the call drops. Phase 1 clear IMBE voice follow is gated by clear grants and reports live sync/NID/LDU/backend diagnostics. Phase 2 TDMA control/grant, slot metadata, NAC/WACN/SYSID tracking, and burst detection are tracked; clear Phase 2 AMBE audio remains muted until TDMA superframe timing, mask application, and MAC/ESS state are complete.
